#!/usr/bin/env python3
"""
=============================================================================
PIPELINE PARALELO: Simulación de EFR para múltiples sujetos
=============================================================================
Este script automatiza la simulación auditiva para una cohorte completa de
pacientes, procesando cada sujeto en paralelo para maximizar el rendimiento.

Pipeline por sujeto:
  1. Leer audiograma individual del paciente (desde Excel)
  2. Convertir audiograma a polos de Shera usando OHC_ind (perfil de pérdida)
  3. Generar estímulo RAM y ejecutar la simulación completa del modelo
  4. Calcular el EFR (suma de armónicos a 110 Hz del ABR simulado)
  5. Almacenar resultados en CSV (nombre del sujeto + valor EFR en µV)

Aplicación clínica:
  Este pipeline permite investigar cómo diferentes perfiles de pérdida
  auditiva afectan la amplitud del EFR, facilitando:
  - Estudios poblacionales de sinapatopatía coclear (sordera oculta)
  - Validación del modelo contra datos clínicos reales
  - Generación de datos sintéticos para entrenar algoritmos de diagnóstico

Creado por: Brent Nissens
Fecha: Octubre 22, 2025
"""

import numpy as np
import os
import sys
import pandas as pd
import multiprocessing as mp
import time
from tqdm import tqdm
import os
import sys

# Agregar carpeta src al path
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'src')))

from utils.get_RAM_stims import get_RAM_stims
from model2018 import model2018
from utils import OHC_ind

def load_shera_poles_profile(sheraP_folder):
    """
    Carga el perfil de polos de Shera desde un archivo StartingPoles.dat.
    
    Los polos definen el grado de amplificación coclear (ganancia de las OHC)
    en cada sección de la cóclea. Un perfil "Flat00" = audición normal;
    otros perfiles representan diferentes grados y configuraciones de
    pérdida auditiva.
    """
    try:
        sheraP_data = np.loadtxt(sheraP_folder + '/StartingPoles.dat')
        if sheraP_data.ndim > 1:
            sheraP = sheraP_data[0, :] 
        else:
            sheraP = sheraP_data
        return sheraP
        
    except Exception as e:
        print(f"Error loading Shera poles profile from {sheraP_folder}: {e}")
        return None

def run_simulation(stim, fs, sheraP, HSR, MSR, LSR):
    """
    Ejecuta una simulación completa del modelo auditivo para un sujeto.
    
    La simulación recorre todo el pipeline:
    Sonido → Cóclea → IHC → AN (HSR+MSR+LSR) → CN → IC → ABR (W1+W3+W5)
    """
    try:
        output = model2018(stim, fs, 'abr', 1, 'evihmlbw', 1, sheraP, 0.05, 'vel', HSR, MSR, LSR, 1, os.getcwd())
        return output
    except Exception as e:
        print(f"Error running simulation: {e}")
        return None

def calculate_EFR(output):
    """
    Calcula el EFR (Envelope Following Response) a partir de la simulación.
    
    El EFR = suma de los 4 primeros armónicos de 110 Hz (frecuencia de
    modulación del estímulo RAM) extraídos de la señal W1+W3+W5,
    convertidos a microvoltios (µV).
    """
    try:
        # Manejar el caso donde la salida es una lista conteniendo un diccionario
        if isinstance(output, list) and len(output) > 0:
            output = output[0]  # Obtener el primer (y probablemente único) elemento
        
        # Extraer las ondas ABR y la frecuencia de muestreo
        fs = float(output.fs_an)
        w1 = output.w1.flatten()
        w3 = output.w3.flatten()
        w5 = output.w5.flatten()

        EFR = w1 + w3 + w5

        # Fourier Transform
        L = len(EFR)
        Y = np.fft.fft(EFR)
        P2 = np.abs(Y / L)
        P1 = P2[:L//2 + 1]
        P1[1:-1] = 2 * P1[1:-1]
        f = fs * np.arange(L//2 + 1) / L

        # Buscar índices de los 4 armónicos de 110 Hz
        fundamental = 110  # Hz
        num_harmonics = 4
        harmonics = np.arange(1, num_harmonics + 1) * fundamental
        idx = []
        for harmonic in harmonics:
            idx.append(np.argmin(np.abs(f - harmonic)))
        # Calcular suma y convertir a microvoltios
        harmonic_sum = np.sum(P1[idx]) * 1e6

        return harmonic_sum

    except Exception as e:
        print(f"Error calculating EFR: {e}")
        return np.nan

def process_single_subject(args):
    """
    Procesa la simulación completa para un único sujeto.
    
    Pipeline por sujeto:
      1. Convertir su audiograma en polos de Shera (OHC_ind)
      2. Ejecutar la simulación con su perfil auditivo individualizado
      3. Calcular su valor de EFR
    
    Parámetros:
    -----------
    args : tuple
        (subject_data, HSR, MSR, LSR, stim, fs, poles_output_dir)
        donde subject_data = (idx, nombre, frecuencias_audiograma, pérdida_dB)
        HSR, MSR, LSR pueden ser escalares o arrays (distribución de fibras AN)
    
    Retorna:
    --------
    dict
        Fila de resultado con nombre del sujeto y valor EFR
    """
    subject_data, HSR, MSR, LSR, stim, fs, poles_output_dir = args
    idx, subject_name, hl_freqs_hz, hl_db = subject_data
    
    try:
        # Crear polos de Shera usando OHC_ind (sin mostrar gráficos)
        OHC_ind.ohc_ind(
            name=subject_name,
            hl_freqs_hz=hl_freqs_hz,
            hl_db=hl_db,
            base_dir=poles_output_dir,
            show_figs=False
        )
        
        # Cargar el perfil de polos generado
        subject_poles_path = os.path.join(poles_output_dir, 'Poles', subject_name)
        sheraP = load_shera_poles_profile(subject_poles_path)
        if sheraP is None:
            print(f"  Could not load poles for {subject_name}, skipping...")
            return None
        
        # Ejecutar simulación con la distribución de fibras AN
        # Note: HSR, MSR, LSR can be scalars (constant across frequency) or 
        # arrays (frequency-dependent ANF distributions)
        output = run_simulation(stim, fs, sheraP, HSR, MSR, LSR)
        
        if output is None:
            return None
        
        # Calcular EFR
        efr = calculate_EFR(output)
        
        # Almacenar resultados
        result_row = {
            'Name': subject_name,
            'EFR': efr
        }
        
        return result_row
        
    except Exception as e:
        print(f"Error processing subject {subject_name}: {e}")
        return None


if __name__ == "__main__":
    # ============================================================================
    # CONFIGURATION - Modify these settings for your use case
    # ============================================================================
    
    # Path to Excel file containing audiogram data
    # Expected format: Excel file with columns 'ID' and 'Audio_XXXHz' (e.g., 'Audio_125Hz', 'Audio_250Hz')
    excel_path = os.path.join(os.path.dirname(__file__), '../data/audiograms.xlsx')
    
    # ---- Configuración de la distribución de fibras AN ----
    # HSR, MSR, LSR: número de fibras de cada tipo por IHC.
    # Valores por defecto (audición normal): HSR=13, MSR=3, LSR=3.
    # Para modelar sinapatopatía coclear (sordera oculta), reducir
    # selectivamente las fibras LSR y/o MSR manteniendo las HSR.
    HSR = 13  # Fibras de alta tasa espontánea (bajo umbral, saturan rápido)
    MSR = 3   # Fibras de media tasa espontánea (umbral intermedio)
    LSR = 3   # Fibras de baja tasa espontánea (alto umbral, importantes en ruido)
    
    # Poles output directory (where OHC_ind will save generated poles)
    poles_output_dir = os.path.join(os.path.dirname(__file__), '../data')
    
    # Stimulus parameters
    fs = 1e5  # Frecuencia de muestreo en Hz
    fRAM = np.array([4000])  # RAM frequency in Hz
    
    # Output settings
    output_csv = 'EFR_results.csv'  # Output CSV filename
    
    # Parallel processing settings
    num_workers = mp.cpu_count()  # Number of parallel workers (default: all CPU cores)
    
    # ============================================================================
    # MAIN PROCESSING PIPELINE
    # ============================================================================
    
    print("="*60)
    print("Starting RAM EFR analysis pipeline...")
    print("="*60 + "\n")
    
    # Cargar datos de Excel
    print("="*60)
    print(f"Loading data from {excel_path}")
    print("="*60 + "\n")

    try:
        df = pd.read_excel(excel_path)
    except Exception as e:
        print(f"Error loading Excel file: {e}")
        sys.exit(1)
    
    # Obtener nombres de columnas de audiograma (formato esperado: Audio_XXXHz)
    audio_columns = [col for col in df.columns if 'Audio_' in col]
    
    if not audio_columns:
        print("Error: No audiogram columns found (expected format: 'Audio_XXXHz')")
        sys.exit(1)
    
    # Inicializar estímulo
    stim = get_RAM_stims(fs, fRAM)
    print("="*60)
    print(f"Generated RAM stimulus with fs={fs} Hz, fRAM={fRAM} Hz")
    print("="*60 + "\n")

    # Preparar datos de todos los sujetos para procesamiento paralelo
    subject_data_list = []
    for idx, row in df.iterrows():
        subject_name = str(row.get('ID', f'Subject_{idx}'))
        
        # Extraer frecuencias y valores del audiograma
        audiogram_data = []
        for col in audio_columns:
            freq_hz = int(col.replace('Audio_', '').replace('Hz', ''))
            hl_db = row[col]
            audiogram_data.append((freq_hz, hl_db))
        
        # Ordenar por frecuencia
        audiogram_data.sort(key=lambda x: x[0])
        hl_freqs_hz = [x[0] for x in audiogram_data]
        hl_db = [x[1] for x in audiogram_data]
        
        subject_data_list.append((idx, subject_name, hl_freqs_hz, hl_db))
    
    print("="*60)
    print(f"Processing {len(subject_data_list)} subjects using {num_workers} workers")
    print(f"ANF distributions: HSR={HSR}, MSR={MSR}, LSR={LSR}")
    print("="*60 + "\n")
    
    # Preparar argumentos para cada worker
    worker_args = [(sd, HSR, MSR, LSR, stim, fs, poles_output_dir) for sd in subject_data_list]
    
    # Procesar sujetos en paralelo
    start_time = time.time()
    completed_subjects = 0
    results = []
    
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        futures = {executor.submit(process_single_subject, args): idx for idx, args in enumerate(worker_args)}
        
        # Recopilar resultados con barra de progreso
        pbar = tqdm(total=len(futures), desc="Processing subjects")
        
        for future in as_completed(futures):
            result = future.result()
            if result is not None:
                results.append(result)
            
            # Actualizar progreso y estimaciones de tiempo
            completed_subjects += 1
            elapsed = time.time() - start_time
            avg_time_per_subject = elapsed / completed_subjects if completed_subjects > 0 else 0
            remaining_subjects = len(futures) - completed_subjects
            estimated_time_left = avg_time_per_subject * remaining_subjects / num_workers if num_workers > 0 else 0
            
            # Formatear estimaciones de tiempo
            elapsed_str = f"{int(elapsed // 3600)}h {int((elapsed % 3600) // 60)}m {int(elapsed % 60)}s"
            eta_str = f"{int(estimated_time_left // 3600)}h {int((estimated_time_left % 3600) // 60)}m {int(estimated_time_left % 60)}s"
            
            pbar.set_postfix({
                'Elapsed': elapsed_str,
                'ETA': eta_str,
                'Avg/Subj': f"{avg_time_per_subject:.1f}s"
            })
            pbar.update(1)
        
        pbar.close()
    
    # Calcular tiempo total de procesamiento
    total_time = time.time() - start_time
    hours = int(total_time // 3600)
    minutes = int((total_time % 3600) // 60)
    seconds = int(total_time % 60)
    
    # Convertir resultados a DataFrame y guardar en CSV
    if results:
        results_df = pd.DataFrame(results)
        results_df.to_csv(output_csv, index=False)
        print(f"\nResults saved to {output_csv}")
        print(f"\nTotal subjects processed: {len(results)}")
        print(f"Total processing time: {hours}h {minutes}m {seconds}s")
        print("\nResults summary:")
        print(results_df)
    else:
        print("\nWarning: No results were generated. Check your configuration and input data.")